...
public class Rank_VideoPlaylistManager : MonoBehaviour
{
    //Player UI 관련
    ...
    //곡 정보 관련
    public Rank_SongInfoUIManager songInfoUIManager; // SongInfoUIManager 참조
    ...
    private List<Rank_VideoPlaylistLoader.SongData> songList; //곡 리스트
    ...
    {
        Rank_VideoPlaylistLoader loader = FindObjectOfType<Rank_VideoPlaylistLoader>();
    ...
    void PlaySong(int index) // 영상의 경로를 찾고 불러오는 메소드
    {
        ...
        // "Videos"폴더로 경로를 설정함
        string filePath = Path.Combine(Application.streamingAssetsPath, "RankAnnounceTracks", "Rank_Videos", videoFileName);
        string imagePath = Path.Combine(Application.streamingAssetsPath, "RankAnnounceTracks", "Rank_Images", imageFileName);
    ...
    void OnVideoPrepared(VideoPlayer vp) // 준비가 완료될 경우 호출되는 메소드
    {
        ...
        if (!imageFileName.EndsWith(".png")) imageFileName += ".png";
        string imagePath = Path.Combine(Application.streamingAssetsPath, "RankAnnounceTracks", "Rank_Images", songData.ImageFileName);
    ...
    IEnumerator PlayRoutine() //코드의 작동 루틴
    {
        ...
        yield return StartCoroutine(FadeOut(UIDisplayTime - fadeDuration));
