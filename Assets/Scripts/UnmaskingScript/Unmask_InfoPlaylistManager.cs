...
public class Unmask_InfoPlaylistManager : MonoBehaviour
{
    public Unmask_InfoUIManager infoUIManager;
    public RawImage FadeUI;

    public Transform identityImage; // 슬라이드할 오브젝트 (UI Image 등)
    public Transform startPos;
    public Transform finishPos;
    public AudioClip revealSFX; //효과음
    ...
    [Header("정체 공개 시간")]
    [SerializeField] private float revealDelay = 4.0f; // 페이드인 후 정체공개까지 대기 시간
    [SerializeField] private float slideDuration = 0.5f; //시작점에서 도착점까지 걸리는 시간

    private List<Unmask_VideoPlaylistLoader.SongData> UnmaskList;
    ...
    {
        identityImage.position = startPos.position; //이미지 위치 초기화
    ...
    IEnumerator SetupBackgroundVideo() //배경 영상을 출력하는 메소드
    {
        ...
        //파일 위치 찾기
        string videoPath = Path.Combine(Application.streamingAssetsPath, "Unmasking", "video.mp4");
        vp.url = videoPath;
    ...
    {
        Unmask_VideoPlaylistLoader loader = FindObjectOfType<Unmask_VideoPlaylistLoader>();

        if (loader == null)
        {
            Debug.LogError("Unmask_listLoader를 씬에서 찾을 수 없음!");
            yield break;
        }

        // CSV 로딩 대기
        float waitTime = 0f;
        while (loader.UnmaskList.Count == 0 && waitTime < 2f)
        {
            yield return new WaitForSeconds(0.1f);
            waitTime += 0.1f;
        }

        UnmaskList = loader.UnmaskList;

        if (UnmaskList.Count > 0)
        {
            currentIndex = 0;
            UpdateCurrentInfo();
    ...
    IEnumerator PlayLoop() //참가자 정보를 순차적으로 자동 표시하는 루프 메소드
    {
        while (true)
        {
            ...
            {
                if (!revealed && timer >= revealDelay)
                {
                    // 정체 공개 시간 도달 → 효과음 재생 + 슬라이드 인
                    if (revealSFX != null) sfxPlayer.PlayOneShot(revealSFX);
                    StartCoroutine(SlideInIdentityImage());
                    revealed = true;
                }
            ...
            currentIndex = (currentIndex + 1) % UnmaskList.Count;
            UpdateCurrentInfo();
            identityImage.position = startPos.position;
    ...
    IEnumerator SlideInIdentityImage() //이미지를 이동시키는 함수
    {
        float t = 0f;
        Vector3 start = startPos.position;
        Vector3 end = finishPos.position;

        while (t < slideDuration)
        {
            float smoothT = Mathf.SmoothStep(0f, 1f, Mathf.Pow(t / slideDuration, 1/3f)); //속도가 점점 느려지며 등장

            identityImage.position = Vector3.Lerp(start, end, smoothT);
            t += Time.deltaTime;
            yield return null;
        }
        identityImage.position = end;
    }

    void UpdateCurrentInfo() //현재 참가자의 정보를 UI에 업데이트하는 메소드
    {
        var song = UnmaskList[currentIndex];
        Texture2D thumbTex = LoadImage(song.ImageFileName);
        Texture2D unmaskTex = LoadImage(song.UnmaskImageFileName);
        infoUIManager.UpdateSongInfo(song.songNumber, song.ComposerName, song.Genre, song.SongTitle, song.Missions, thumbTex, unmaskTex);
    }

    void Jump(int index) // 특정 인덱스로 점프+페이드 아웃/인
    {
        if (isTransitioning || UnmaskList == null || UnmaskList.Count == 0) return;
    ...
    IEnumerator JumpToIndex(int newIndex)
    {
        ...
        //다음으로 넘어감
        currentIndex = Mathf.Clamp(newIndex, 0, UnmaskList.Count - 1);
        UpdateCurrentInfo();
        identityImage.position = startPos.position; //이미지 위치 초기화
    ...
    Texture2D LoadImage(string imageName) // 이미지 파일을 로드하여 Texture2D로 반환
    {
        ...
        string path = Path.Combine(Application.streamingAssetsPath, "Unmasking", "Unmask_Images", imageName);
