...
public class Rank_VideoPlaylistLoader : MonoBehaviour
{
    [System.Serializable]
    ...
    void LoadCSV()
    {
        string filePath = Path.Combine(Application.streamingAssetsPath, "RankAnnounceTracks", "Rank_playlist.csv");
            ...
            using (StreamReader reader = new StreamReader(filePath))
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    SongData songData = new SongData
                    {
                        ...
                        videoFileName = row[1].Trim(),
                        ImageFileName = row[2].Trim(),
                        ComposerName = row[3].Trim(),
                        SongTitle = row[4].Trim(),
                        Genre = row[5].Trim(),
                        Missions = row[6].Trim()
                    };
